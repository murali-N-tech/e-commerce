...
const ProductManagePage = ({ onNavigate }) => {
  ...
  return (
    <div className="container mx-auto px-6 py-12">
      <h2 className="text-5xl font-extrabold text-gray-800 mb-12 text-center">Manage Products</h2>

      <div className="bg-white rounded-xl shadow-2xl p-8 md:p-10 mb-12">
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          ...
          {!editingProduct && (
            <button
              ...
              className="bg-indigo-600 text-white px-6 py-3 rounded-full text-lg font-semibold shadow-md hover:bg-indigo-700 transition-colors duration-200 flex items-center space-x-2"
            >
              <PlusCircle size={20} />
          ...
          {editingProduct && (
             <button
                onClick={resetForm}
                className="bg-gray-400 text-white px-6 py-3 rounded-full text-lg font-semibold shadow-md hover:bg-gray-500 transition-colors duration-200 flex items-center space-x-2"
              >
                <XCircle size={20} />
        ...
        {(isAddingNew || editingProduct) && (
          <form onSubmit={handleCreateOrUpdateProduct} className="space-y-6">
            {formMessage && (
              <div className={`p-4 rounded-lg text-center ${formMessage.includes('successfully') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                {formMessage}
              </div>
            )}
            <div>
              ...
              <input
                type="text"
                id="name"
                className="shadow appearance-none border rounded w-full py-3 px-4 bg-white text-gray-800 leading-tight focus:outline-none focus:shadow-outline focus:ring-2 focus:ring-indigo-500"
                value={name}
            ...
            <button
              type="submit"
              className="mt-6 w-full bg-green-600 text-white px-6 py-3 rounded-full text-lg font-semibold shadow-md hover:bg-green-700 transition-colors duration-200 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <h3 className="text-3xl font-bold text-gray-800 mb-8 text-center">Existing Products</h3>
      {loadingProducts ? (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {products.map((product) => (
                <tr key={product._id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 truncate w-24">{product._id}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button
                      ...
                      className="text-indigo-600 hover:text-indigo-900 mr-3 p-2 rounded-full hover:bg-indigo-100 transition-colors"
                      title="Edit Product"
                    >
                      <Edit size={18} />
                    </button>
